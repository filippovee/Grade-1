...
class Field
{
    ...
    private function rout($text, $id, $type_obj, $name)
    {
        ...
        //Проверяем наличие поля типа текст
        $field_id = self::checkfield($type);
        if ($field_id != NULL) {
            //Если поле есть, присваиваем ему значение
            self::add_field($text, $field_id, $id, $type);
        }else{
            //Если поля нет, создаём его и добавляем значение.
            self::create_field($text, $id, $type, $name, $type_obj);
          //  var_dump($name);
    ...
    function create_field($text, $id, $type, $name, $type_obj){
        $arr=[ "name"	=> $name,
            "type"	=> "1",
